...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
    ...
    [GenerationMode.CLONE_VIDEO]: <SparklesIcon className="w-5 h-5" />,
};
    ...
    <div>
        ...
        <div className="relative">
            ...
            <select
                ...
                onChange={onChange}
                disabled={disabled}
                className="w-full bg-[#1f1f1f] border border-gray-600 rounded-lg pl-10 pr-8 py-2.5 appearance-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-700/50 disabled:border-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white">
                {children}
            </select>
            <ChevronDownIcon
...
interface PromptFormProps {
    onGenerate: (params: GenerateVideoParams & { model: KieModel }) => void;
    initialValues?: (GenerateVideoParams & { model?: KieModel }) | null;
}
...
const PromptForm: React.FC<PromptFormProps> = ({
    ...
    const [model, setModel] = useState<KieModel>(
        initialValues?.model ?? KieModel.VEO_FAST,
    );
    ...
    useEffect(() => {
        if (initialValues) {
            ...
            setModel(initialValues.model ?? KieModel.VEO_FAST);
    ...
    const promptPlaceholder = {
        ...
        [GenerationMode.CLONE_VIDEO]: 'Describe how to evolve the source video...',
    }[generationMode];
    ...
    switch (generationMode) {
        ...
        case GenerationMode.EXTEND_VIDEO:
            isSubmitDisabled = !prompt.trim() || !inputVideo;
            if (isSubmitDisabled) {
                tooltipText = 'Prompt and input video are required.';
            }
            break;
        case GenerationMode.CLONE_VIDEO:
            isSubmitDisabled = !prompt.trim();
            if (isSubmitDisabled) {
                tooltipText = 'Please enter a prompt.';
            }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex flex-col">
                            <CustomSelect
                                label="Model"
                                value={model}
                                onChange={(e) => setModel(e.target.value as KieModel)}
                                icon={<SparklesIcon className="w-5 h-5 text-gray-400" />}>
                                {Object.values(KieModel).map((modelValue) => (
                                    <option key={modelValue} value={modelValue}>
                                        {modelValue === KieModel.VEO_QUALITY ? 'Veo 3.1 Quality' : 'Veo 3.1 Fast'}
                                    </option>
                                ))}
                            </CustomSelect>
                        </div>
                ...
                <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
                    ...
                    <div className="relative group">
                        ...
                        {isSubmitDisabled && tooltipText && (
                            <div
                                role="tooltip"
                                className="absolute bottom-full right-0 mb-2 w-max max-w-xs px-3 py-1.5 bg-gray-900 border border-gray-700 text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 text-center">
                                {tooltipText}
                            </div>
                        )}
                    </div>
